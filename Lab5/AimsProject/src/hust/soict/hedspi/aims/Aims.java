...
public class Aims {
    ...
    private static void addMediaToCartSafe(Media media) {
        try {
            cart.addMedia(media);
        } catch (LimitExceededException e) {
            System.out.println("Khong the them media: " + e.getMessage());
            JOptionPane.showMessageDialog(null, e.getMessage(), "Cart Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private static void playMediaSafe(Playable playable) {
        try {
            playable.play();
